...
const mainDb = () => {
    ...
    const get = (input: any) => {
        ...
        const list = Object.keys(res).reduce((a: any, k) => {
            const items = res[k]
        ...
        if (input.pk2) {
            const res = list.filter(
                (x: any) => x.pk2 === input.pk2 && x.sk === input.sk
            )
            return res[0] || false
        }
        if (input.pk3) {
            const res = list.filter(
                (x: any) => x.pk3 === input.pk3 && x.sk === input.sk
            )
            return res[0] || false
        }
        if (input.pk) {
            const res = list.filter(
                (x: any) => x.pk === input.pk && x.sk === input.sk
            )
            return res[0] || false
        }
        return false
    ...
    const list = (input: any) => {
        ...
        if (input.pk2) {
            const res = list.filter(
                (x: any) => x.pk2 === input.pk2 && x.sk.startsWith(input.sk)
            )
            return res || []
        }
        if (input.pk3) {
            const res = list.filter(
                (x: any) => x.pk3 === input.pk3 && x.sk.startsWith(input.sk)
            )
            return res || []
        }
        if (input.pk) {
            const res = list.filter(
                (x: any) => x.pk === input.pk && x.sk.startsWith(input.sk)
            )
            return res || []
        }
        return []
